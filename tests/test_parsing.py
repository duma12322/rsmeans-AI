...
def test_clean_json():
    out = _parse_ranking('{"ranked":["26","23"],"confidence":"high","clarify":null}', VALID)
    assert out == {"ranked": ["26", "23"], "confidence": "high", "clarify": None}


def test_fenced_json():
    raw = '```json\n{"ranked":["26"],"confidence":"medium","clarify":null}\n```'
...
def test_clarify_passthrough():
    raw = '{"ranked":["22","23"],"confidence":"low","clarify":"Is the pump for plumbing or HVAC?"}'
    out = _parse_ranking(raw, VALID)
    assert out["clarify"] == "Is the pump for plumbing or HVAC?"
    assert out["confidence"] == "low"
